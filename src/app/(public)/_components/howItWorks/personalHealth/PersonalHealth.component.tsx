...
export default function PersonalHealth() {
  ...
  //   const seed = year * 10 + CURRENT_QUARTER
  //   const r = seededRandom(seed)

  //   const active = r < 0.33 ? 'optimal' : r < 0.66 ? 'average' : 'attention'

  //   return {
  //     year,
  //     optimal: active === 'optimal' ? Q_POSITION.Q1 : null,
  //     average: active === 'average' ? Q_POSITION.Q2 : null,
  //     attention: active === 'attention' ? Q_POSITION.Q3 : null,
  //   }
  // })

  const jitter = (seed: number, base: number) => {
    const r = seededRandom(seed)
    return +(base + (r - 0.5) * 0.4).toFixed(2) // ±0.2 jitter
  }

  // const chartData = YEARS.map((year) => {
  //   const baseSeed = year * 100 + CURRENT_QUARTER * 10

  //   return {
  //     year,
  //     optimal: jitter(baseSeed + 1, 1), // Q1
  //     average: jitter(baseSeed + 2, 2), // Q2
  //     attention: jitter(baseSeed + 3, 3), // Q3
  //   }
  // })
  const QUARTER_INDEX = {
    Q1: 1,
  ...
  const chartData = YEARS.map((year) => {
    ...
    const allow = (q: number) => !isCurrentYear || q <= CURRENT_QUARTER

    return {
      year,
      isCurrentYear,
      optimal: allow(1) ? jitter(baseSeed + 1, 1) : null,
      average: allow(2) ? jitter(baseSeed + 2, 2) : null,
      attention: allow(3) ? jitter(baseSeed + 3, 3) : null,
    }
  })
            ...
            <Card className="bg-white p-4 sm:p-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-green-500 text-white text-center p-2 rounded text-sm font-medium">
                    <span className="text-lg">75</span>
                    <span className="text-xs">Optimal Range</span>
                  </div>
                  <div className="bg-orange-500 text-white text-center p-2 rounded text-sm font-medium">
                    <span className="text-lg">10</span>
                    <span className="text-xs">Average Range</span>
                  </div>
                  <div className="bg-red-500 text-white text-center p-2 rounded text-sm font-medium">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                ...
                <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  {/* <XAxis dataKey="year" type="number" domain={[2021, 2025]} tickCount={5} padding={{ left: 20, right: 20 }} /> */}
                  <XAxis dataKey="year" type="number" domain={[minYear, maxYear]} ticks={chartData.map((d) => d.year)} padding={{ left: 20, right: 20 }} />

                  <YAxis type="number" domain={[1, 3]} ticks={[1, 2, 3]} tickFormatter={(v) => (v === 1 ? 'Q1' : v === 2 ? 'Q2' : 'Q3')} padding={{ top: 20, bottom: 20 }} />

                  {/* <CartesianGrid strokeDasharray="3 3" /> */}
                  {/* <Tooltip /> */}

                  {/* Optimal (green) */}
                  {/* <Line data={optimal} type="monotone" dataKey="value" stroke={COLORS.optimal} strokeWidth={3} fill={COLORS.optimal} dot={{ r: 3 }} connectNulls /> */}

                  {/* Average (orange) */}
                  {/* <Line data={average} type="monotone" dataKey="value" stroke={COLORS.average} fill={COLORS.average} strokeWidth={3} dot={{ r: 3 }} connectNulls /> */}
